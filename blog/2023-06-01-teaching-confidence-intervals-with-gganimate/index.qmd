...
2. Second, I use a computer to produce a plot of many (about 50 seems right) confidence intervals from the same repeated study. I explain that the interval we will get in the study we actually conduct is like a random draw from this collection. (See below for an example of this figure.)
3. Third, I make the plot *dynamic*. I find that dynamics make the plot more memorable and convey the (appropriate) idea that hypothesis tests and confidence intervals are chaotic, noisy quantities.
...
#| fig-height: 6
#| fig-width: 6
#| fig-alt: "A scatterplot of 50 simulated point estimates with 90% confidence interval error bars, one row per study, for a study with 80% power (true effect 1, SE 0.4). A dotted vertical line marks the true effect and a solid vertical line marks zero. Intervals are colored orange when they include zero (fail to reject the null) and green when they exclude it (reject the null); several of the 50 intervals overlap zero."
...
#| fig-width: 6
#| cache: true
#| fig-alt: "The same 50-study confidence-interval plot as above (80% power, colored green for reject-null and orange for fail-to-reject), redrawn with extra padding on the x- and y-axes to leave room for points to animate into place."
...
#| fig-width: 8
#| cache: true
#| fig-alt: "Animated version of the 50-study confidence-interval plot for a study with 80% power: each study's estimate and 90% confidence interval drifts in from the right, one at a time, colored black on entry and then recoloring to green (reject the null) or orange (fail to reject), with earlier studies left behind as faded shadow marks."
...
# make magic happen!
animate(anim, duration = 12, fps = 15,
        height = 6, width = 8, units = "in", res = 90)
```
...
#| include: false
animate(anim, duration = 12, fps = 15,
        height = 4, width = 8, units = "in", res = 90)
...
#| fig-width: 8
#| cache: true
#| fig-alt: "Animated version of the 50-study confidence-interval plot for a study with about 98% power (SE 0.27): each study's estimate and 90% confidence interval drifts in from the right and recolors to green (reject the null) or orange (fail to reject); almost every interval ends up green, in contrast to the 80%-power animation above where several stay orange."
