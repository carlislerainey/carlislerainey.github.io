...
twitter-card:
  ...
  title: "Statistical Power from Pilot Data: Simulations"  # less than 55 chars
  description: "In this post, I discuss how pilot data can be used to predict the standard error in a planned study."  # less than 125 chars
open-graph:
  image: "twitter-card.png"
  title: "Statistical Power from Pilot Data: Simulations"  # less than 55 chars
  description: "In this post, I discuss how pilot data can be used to predict the standard error in a planned study."  # less than 125 chars
...
Note that the factor $\sqrt{\frac{1}{n^{pilot}}} + 1$ nudges the predicted standard error in a conservative direction. See [this working paper](https://osf.io/preprints/osf/5am9q_v3) for more details.
...
```{r}
#| message: false
#| fig-alt: "Five histograms, one per pilot sample size (10, 30, 60, 90, 150 respondents per condition), showing the distribution of the estimated standard error across 10,000 simulated pilot studies. A vertical line marks the true standard error of the planned 500-per-condition study. In every panel the pilot standard errors are centered above that line, and the smaller the pilot, the more the distribution spreads out and overshoots it."
...
```{r}
#| message: false
#| fig-alt: "Five histograms, one per pilot sample size, showing the pilot standard error rescaled by sqrt(n_pilot/n_planned) to predict the standard error of the planned 500-per-condition study. A vertical line marks the true planned-study standard error; the rescaled predictions are now centered right on that line in every panel, though the distribution still spreads below it (an underestimate) for the smallest pilots."
...
As a solution, we can gently nudge the standard error from the pilot up by a factor of $\left( \sqrt{\frac{1}{n^{pilot}}} + 1 \right)$, which will make "almost all" of the standard errors *over*-estimates or "conservative" (details [here](https://osf.io/preprints/osf/5am9q_v3)).

```{r}
#| message: false
#| fig-alt: "Five histograms, one per pilot sample size, showing the rescaled pilot standard error after the conservative nudge factor (sqrt(1/n_pilot) + 1) is applied. A vertical line marks the true planned-study standard error; the nudged predictions now sit almost entirely at or above that line in every panel, so the standard error is rarely underestimated."
...
```{r}
#| message: false
#| fig-alt: "Five histograms, one per pilot sample size, showing the distribution of the conservative minimum detectable effect (2.5 times the predicted standard error) implied by each simulated pilot. A vertical line marks the true treatment effect of 1; the minimum detectable effect distributions cluster near that line and shift slightly higher and wider for smaller pilots."
...
```{r}
#| message: false
#| fig-alt: "Five histograms, one per pilot sample size, showing the distribution of the conservative power estimate implied by each simulated pilot for a treatment effect of 1. A vertical line marks the target of 80% power; the power distributions cluster around and mostly above that line, with more spread for smaller pilots."
# compute the power
pwr <- res %>%
  mutate(pred_se_cons = sqrt(n_pilot/n_planned)*(sqrt(1/n_pilot) + 1)*pilot_se_hat, 
...
```{r}
#| message: false
#| fig-alt: "Five histograms, one per pilot sample size, showing the distribution of the conservative required sample size per condition implied by each simulated pilot, for 80% power to detect a treatment effect of 1. A vertical line marks the true required sample size of 500; the distributions cluster above that line, meaning the conservative rule usually recommends more respondents than strictly necessary, and the excess shrinks as the pilot gets larger."
...
You can find more details in [this paper](https://osf.io/preprints/osf/5am9q_v3).
