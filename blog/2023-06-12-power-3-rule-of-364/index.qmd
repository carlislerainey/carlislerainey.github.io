...
Predicting the standard error is a mechanical process mixed with a little guesswork.^[I'll suggest two methods I like. First, use $\text{predicted SE} = \frac{\text{SD of same outcome in different data set}}{2 \sqrt{\text{sample size}}}$. I like to confirm this estimate with a small pilot. I sample observations from this pilot data set, run the full analysis, and confirm that my prediction is close, and make any needed adjustments.] Choose a true effect is a mostly substantive decision.^[[Cyrus Samii](https://twitter.com/cdsamii/status/1661091885580992517?s=20) likes to use the MME, I like a conservatively choosen guess of what the effect actually is.]
