...
[^1]: I'm focusing on confidence intervals here because inference from confidence intervals is a bit more intuitive (see [Rainey 2014](http://www.carlislerainey.com/papers/nme.pdf) and [Rainey 2015](http://www.carlislerainey.com/papers/meaningful.pdf). In the cases I discuss, whether one checks whether the *p*-value is less than 0.05 or checks that the confidence interval contains zero are equivalent.
...
Again, you should apply this standard to a *literature*. You should not apply this standard to a particular study because even well-powered studies sometimes produce confidence intervals that nestle up against zero. But when you start to see confidence intervals consistently falling close to zero, you should start to assume that the literature uses under-powered studies and that the estimates in that literature are inflated due to Type M errors [(Gelman and Carlin 2014)](https://journals.sagepub.com/doi/10.1177/1745691614551642).

```{r, fig.height=10, fig.width=6}
#| fig-alt: "Four stacked panels, one per power level (25%, 50%, 80%, 95%), each showing 25 simulated 90% confidence intervals as horizontal error bars ordered by estimate. A solid vertical line marks zero, a dotted line marks one standard error above zero, and a dashed line marks the true effect of 0.5. Intervals are colored orange when their lower bound clears one standard error above zero and green when it does not. In the 25%-power panel almost every interval is green (nestled against zero); in the 95%-power panel almost every interval is orange (not nestled), showing that well-powered studies produce confidence intervals that sit further from zero."
...
#| fig-alt: "Four density plots, one per power level (25%, 50%, 80%, 95%), showing the distribution of the lower bound of the 90% confidence interval among significant estimates. A dotted vertical line marks one standard error above zero in each panel. At 25% power the density piles up just to the right of zero (nestled against zero); as power rises to 95% the density shifts further to the right, away from the dotted line."
