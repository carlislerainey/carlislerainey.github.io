...
This post turned out to be somewhat popular, so I've written up a more formal, careful description of the idea in a full-length paper. You can find the preprint "Power Rules" [here](https://osf.io/preprints/osf/5am9q_v3).
:::
...
We can visualize this with gganimate. We can imagine each iteration of the study as producing a particular estimate. We continue to repeat the study and collect the estimates. Eventually, we can produce a histogram from this collection of estimates. This histogram represents the sampling distribution and is fundamental to the calculations that follow. The figure belows shows how we might collect the points into a histogram.

```{r}
#| column: screen-inset-right
#| warning: false
#| message: false
#| fig-alt: "Side-by-side animation. The left panel builds a scatterplot of 50 simulated study estimates (dollars) one point at a time, with dashed reference lines at the true effect and at 1, 2, and 3 standard errors above and below it, each labeled with its share of the normal distribution. The right panel simultaneously builds a histogram of the same accumulating estimates. Together they show individual draws from a sampling distribution assembling into the bell-shaped histogram that the distribution predicts."
...
for(i in 2:min(length(gg1_mgif), length(gg2_mgif))){  # gganimate may adjust nframes to fit the states
...
#| warning: false
#| message: false
#| fig-alt: "Side-by-side animation. The left panel adds 50 simulated study estimates one at a time as points with 90% confidence interval error bars, colored green when the interval's lower bound clears zero (reject the null) and orange when it does not; a vertical line marks 1.64 standard errors above zero. The right panel simultaneously builds a bar chart tallying how many of the accumulated studies rejected the null versus failed to reject it, ending with most studies rejecting but a visible minority not."
...
#| fig-height: 5
#| fig-width: 10
#| fig-alt: "A normal density curve for the sampling distribution of the estimate (true effect 1, SE 0.4), with dashed reference lines at the true effect and at multiples of the SE. A vertical green line marks 1.64 SEs above zero, the threshold for rejecting the null, and the orange-shaded area to its right (the tail of the distribution beyond that threshold) is labeled 'fraction rejected,' showing what portion of the sampling distribution clears the rejection threshold."
...
ggplot() + 
  ...
  geom_function(fun = dnorm, args = list(mean = true_effect, sd = se), linewidth = 1) + 
  theme_bw() + 
  labs(x = "Estimate of Effect",
       y = "Density") + 
  ...
  annotate("segment", x = 1.64*se, xend = 1.64*se, y = 0, yend = dnorm(1.64*se, mean = true_effect, sd = se), color = "#1b9e77", linewidth = 1) + 
...
#| fig-height: 5
#| fig-width: 10
#| fig-alt: "A normal density curve for the sampling distribution when the true effect is 3.64 standard errors above zero (SE = 1/3.64). Annotated segments mark the true effect, 1.64 SEs above zero (the rejection threshold), the 1.64-SE width of the 90% confidence interval, the roughly 2-SE gap researchers should aim to add beyond that threshold, and the full 3.64-SE distance from zero to the true effect, illustrating how the two pieces (1.64 + 2) combine into the rule of 3.64."
...
ggplot() + 
  ...
  geom_area(data = tibble(x = seq(1.64*se, 3*se + true_effect, by = 0.1)), aes(x = x), 
            ...
            fill = "#d95f02", alpha = 0.1, xlim = c(1.64*se, 3**se + true_effect)) + 
  #annotate("label", x = 1.3, y = .1, label = "fraction rejected", color = "#d95f02", size = 6) +
  annotate("segment", x = true_effect, xend = true_effect, y = 0, yend = dnorm(true_effect, mean = true_effect, sd = se), color = "#d95f02", linewidth = 1) + 
  annotate("label", x = true_effect, y = dnorm(true_effect, mean = true_effect, sd = se)/2, label = "true effect", color = "#d95f02") +   
  annotate("segment", x = 1.64*se, xend = 1.64*se, y = 0, yend = .75, color = "#1b9e77", linewidth = 1) + 
